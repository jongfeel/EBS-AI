...
weekday_title = ['MON', 'TUE', 'WED', 'THR', 'FRI']     # 주중
...
for hour_index, hour_value in enumerate(hour_title) :                          # 0~23시간만큼 19~24행 반복                     
    ...
    # 각 시간대 주간 총합
    for day_index, day_value in enumerate(weekday_title) :                      # 일주일, 즉 7번 반복하기
        day_sum += int(a[day_index][hour_index]['num'])  # 요일별 + 시간대별 여성 행인수 누적하기

    avgh.append(day_sum/7)                       # 시간대 주간 여성 행인수 평균 구하기
